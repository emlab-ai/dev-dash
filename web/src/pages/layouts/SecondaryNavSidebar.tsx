...
const ListElement = ({ text, url, selected, icon}: { text: string, url:string, selected:boolean, icon?:any }) => {
    return (
        <Link to={url}>
                <ListItem as={HStack} spacing={0} h="10" pl="2.5" cursor="pointer" _hover={{ bg: useColorModeValue('gray.100', 'gray.700') }} rounded="md" background={ selected? useColorModeValue('gray.200', 'gray.500'): undefined}>
