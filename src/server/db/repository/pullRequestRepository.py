...
class PullRequestStats:
    def __init__(
        self, count, avg_loc, avg_duration, avg_files_changed, avg_comments_count
    ):
        self.count = count
...
class PullRequestRepository:
    ...
    def list_all(
        ...
        start_date,
        end_date,
        github_users_ids: list[int] = None,
        github_repo_id=None,
        limit: int = None,
        after=None,
        before=None,
        ...
        totalDurationFunc = cast(
            func.round(
                func.extract(
                    "epoch", PullRequest.closed_at - PullRequest.first_commit_date
                )
                / 3600,
                2,
            ),
        ...
        sorting_keys = {
            "id": {"id": "id", "attr": PullRequest.id},
            "closed_at": {"id": "closed_at", "attr": PullRequest.closed_at},
            "totalDuration": {"id": "total_duration", "attr": totalDurationFunc},
            "changes": {"id": "changes", "attr": changesFunc},
            "changedFiles": {"id": "changed_files", "attr": PullRequest.changed_files},
            "reviewThreadsCount": {
        ...
        if sort_by and (sort_by not in sorting_keys):
        ...
        query = query.filter(
            func.date(PullRequest.closed_at) >= start_date.date(),
            func.date(PullRequest.closed_at) <= end_date.date(),
        )
        ...
        before_cursor = (
            encode_cursor(
                ...
                sort_by,
                before_cursor_item[sort_by_key] if sort_by_key else None,
            )
        ...
        after_cursor = (
            encode_cursor(
                ...
                sort_by,
                after_cursor_item[sort_by_key] if sort_by_key else None,
            )
    ...
    def get_pr_stats_group_by_user(
        self,
        tenant_id: int,
        start_date,
        end_date,
        ...
        sort_by: str = None,
        sort_order: str = None,
    ) -> PagedResult[PullRequestsUsersStats]:
        ...
        avgDurationFunc = func.avg(
            func.coalesce(PullRequest.closed_at, func.current_date())
            - func.coalesce(PullRequest.first_commit_date, func.current_date())
        )
        maxDurationFunc = func.max(
            func.coalesce(PullRequest.closed_at, func.current_date())
            - func.coalesce(PullRequest.first_commit_date, func.current_date())
        )

        sorting_keys = {
            "id": {"id": "id", "attr": GithubUser.id},
            "name": {"id": "user_name", "attr": User.name},
            "count": {"id": "count", "attr": countFunc},
            "avg_loc": {"id": "avg_loc", "attr": avgLocFunc},
            "sum_loc": {"id": "sum_loc", "attr": sumLocFunc},
            "max_loc": {"id": "max_loc", "attr": maxLocFunc},
            "avg_duration": {"id": "avg_duration", "attr": avgDurationFunc},
            "max_duration": {"id": "max_duration", "attr": maxDurationFunc},
        }
    ...
    def get_pr_stats_group_by_repo(
        self,
        tenant_id: int,
        start_date,
        end_date,
        ...
        sorting_keys = {
            "id": {"id": "id", "attr": GithubRepo.id},
            "name": {"id": "name", "attr": GithubRepo.name},
            "count": {"id": "count", "attr": countFunc},
            "avg_loc": {"id": "avg_loc", "attr": avgLocFunc},
            "sum_loc": {"id": "sum_loc", "attr": sumLocFunc},
            "max_loc": {"id": "max_loc", "attr": maxLocFunc},
    ...
    def get_avg_stats(
        self, tenant_id: int, start_date, end_date, github_user_ids: list[int]
    ) -> PullRequestStats:
        query = self.session.query(PullRequest)
        query = query.filter(PullRequest.tenant_id == tenant_id)
        query = query.filter(
            PullRequest.closed_at >= start_date, PullRequest.closed_at <= end_date
        )
        query = query.filter(
            ~PullRequest.repository_id.in_(
                self.session.query(RepoSettings.repository_id).filter(
        ...
        query = query.with_entities(
            ...
            func.avg(PullRequest.closed_at - PullRequest.first_commit_date).label(
                "avg_duration"
            ),
    ...
    def get_count_by_date_user(
        ...
        query = query.filter(
            PullRequest.closed_at >= start_date,
            PullRequest.closed_at <= end_date,
            PullRequest.author_id == github_user_id,
        )
        query = query.with_entities(
            func.date_trunc("day", PullRequest.closed_at).label("closed_day"),
            func.count(PullRequest.id).label("count"),
        )
