...
from db.model import GithubPullRequestReview, User, GithubUser
...
class PullRequestReviewRepository:
    ...
    def list_all(self, tenant_id:int, user_id:int, start_date:datetime, end_date:datetime, limit=None, after=None, before=None):
        ...
        query = query.filter(GithubPullRequestReview.submitted_at >= start_date.date(), GithubPullRequestReview.submitted_at <= end_date)
        query = query.join(GithubUser, GithubUser.id == GithubPullRequestReview.author_id)
    ...
    def get_count_by_date_user(self, tenant_id:int, user_id:int, start_date:datetime, end_date:datetime):
        query = self.session.query(GithubPullRequestReview)
        query = query.filter(GithubPullRequestReview.tenant_id == tenant_id)
        query = query.filter(GithubPullRequestReview.submitted_at >= start_date, GithubPullRequestReview.submitted_at <= end_date, GithubPullRequestReview.author_id == user_id)
        query = query.with_entities(func.date_trunc('day', GithubPullRequestReview.submitted_at).label('created_day'), 
                                    func.count(GithubPullRequestReview.id).label('count')) 
        query = query.group_by(func.date_trunc('day', GithubPullRequestReview.submitted_at))
