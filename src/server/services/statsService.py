...
def percent_difference(a, b):
    return round(((a - b) / b), 2) if b > 0 else 0
...
class StatsService:
    ...
    def build_cues(
        self, tenant_id: int, managerId: int, start_date: datetime, end_date: datetime
    ):
        ...
        if managerId is not None:
            ...
            user_ids = userService.get_github_user_for_manager_ids(
                tenant_id, manager_ids
            )

        if managerId is not None:
            reports = userService.get_all_reports(
                tenant_id, managerId, directOnly=False
            )
        ...
        result_prev = prRepository.list_all(
            tenant_id=tenant_id,
        ...
        diffLess100 = len(
            [pr for pr in result if pr["deletions"] + pr["additions"] <= 100]
        )
        diffLess100_prev = len(
            [pr for pr in result_prev if pr["deletions"] + pr["additions"] <= 100]
        )
        ...
        return [
            ...
            },
            {
                ...
                "diffPercent": percent_difference(
                    round(
                        (
                            (len(result_prev) / total_swes_prev)
                            if total_swes_prev > 0
                            else 0
                        ),
                        2,
                    ),
                    round((len(result) / total_swes) if total_swes > 0 else 0, 2),
                ),
            },
            {
                "label": "Active SWEs",
    ...
    def build_line_charts(
        ...
        result = prRepository.list_all(
            ...
            start_date=start_date,
            end_date=end_date,
            github_users_ids=github_user_ids,
        )
        result = result.data if result else []
        unique_swes = len(set([pr["author_id"] for pr in result]))
        ...
        for pr in result:
            ...
            less100_grouped_by_date[closed_date] += (
                1 if pr["deletions"] + pr["additions"] <= 100 else 0
            )
        ...
        for pr in result:
            ...
            average_duration_per_day[closed_date].append(
                pr["total_duration"] if pr["total_duration"] else 0
            )
                ...
                "datasets": [
                    {
                        ...
                        "data": [
                            (
                                grouped_by_date.get(date, 0) / total_swes
                                if total_swes > 0
                                else 0
                            )
                    ...
                    },
                    {
                        ...
                        "data": [
                            (
                                (grouped_by_date.get(date, 0) / unique_swes)
                                if unique_swes > 0
                                else 0
                            )
                            for date in date_range
            ...
            },
            {
                ...
                "datasets": [
                    ...
                    },
                    {
                        ...
                        "data": [
                            len(active_swe_by_date.get(date, set()))
                            for date in date_range
                        ],
                    },
                    {"label": "SWE Count", "data": [total_swes for date in date_range]},
                ],
            },
            {
                "id": "timePerDiff",
                ...
                "datasets": [
                    {
                        ...
                        "data": [
                            average_duration_per_day.get(date, 0) for date in date_range
                        ],
                    },
                    {
                        "label": "P80: First commit to close",
                        "data": [
                            p80_duration_per_day.get(date, 0) for date in date_range
                        ],
                    },
                ],
            },
                ...
                "datasets": [
                    {
                        ...
                        "data": [
                            (
                                round(
                                    len(active_swe_by_date.get(date, [])) / total_swes,
                                    2,
                                )
                                if total_swes > 0
                                else 0
                ...
                "datasets": [
                    {
                        "label": "Diffs <= 100 LoC",
                        "data": [
                            less100_grouped_by_date.get(date, 0)
                            / grouped_by_date.get(date, 1)
                            for date in date_range
                        ],
                    }
                ],
    ...
    def build_user_stats(
        ...
        result = prRepository.get_pr_stats_group_by_user(
            tenant_id,
            start_date,
            end_date,
            github_user_ids,
    ...
    def build_repo_stats(
        ...
        result = prRepository.get_pr_stats_group_by_repo(
            tenant_id,
            start_date,
            end_date,
            github_user_ids,
