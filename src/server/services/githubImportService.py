...
from services.githubClient import get_org_repos, get_repo_pull_requests2, github_gql_query
...
class GithubImportService:
    ...
    def send_event(self, event_type, tenant_id, installation_id, data):
        if self.inprocess:
            ...
            publish_to_kinesis(app_config.IMPORT_STREAM_ARN, str(tenant_id), {"event_type": event_type, "tenant_id": tenant_id, "installation_id": installation_id, "data": data})
    ...
    def create_import_request(self, tenant, org):                
        ...
        while(True):
            ...
            for repo in repositories:
                ...
                self.send_event("import_repository", tenant.id, org.installation_id, {"repo_full_name": repo.nameWithOwner})
    ...
    def _fetch_org_users(self, tenant_id, installation_id, org_name, cursor=None):
        ...
        ids = [user.databaseId for user in result.data.organization.membersWithRole.nodes]
        ...
        if pageInfo.hasNextPage:
            self._fetch_org_users(tenant_id, installation_id, org_name, pageInfo.endCursor)
