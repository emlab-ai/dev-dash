...
class TestEventHelpers(unittest.TestCase):
    def test_pull_request_review_comment_to_model(self):
        with open(
            get_test_file_path("pull_request_review_comment[created].json"), "r"
        ) as f:
            ...
            self.assertEqual(
                model.reactions_count, data["comment"]["reactions"]["total_count"]
            )
            ...
            self.assertEqual(
                model.pr_review_id, data["comment"]["pull_request_review_id"]
            )
            self.assertEqual(model.commit_id, data["comment"]["commit_id"])
            self.assertEqual(
                model.author_association, data["comment"]["author_association"]
            )
    ...
    def test_pull_request_event_to_model(self):
        with open(get_test_file_path("pull_request[opened].json"), "r") as f:
            ...
            self.assertEqual(
                pull_request.created_at, data["pull_request"]["created_at"]
            )
            self.assertEqual(
                pull_request.changed_files, data["pull_request"]["changed_files"]
            )
            ...
            self.assertEqual(
                pull_request.commits_count, data["pull_request"]["commits"]
            )
            self.assertEqual(
                pull_request.review_threads_count,
                data["pull_request"]["review_comments"],
            )
            self.assertEqual(
                pull_request.comments_count, data["pull_request"]["comments"]
            )
    ...
    def test_repository_to_model(self):
        with open(
            get_test_file_path("installation_repositories[added].json"), "r"
        ) as f:
            event_data = json.load(f)
            data = event_data["payload"]
            tenant = Tenant(id=1, name="test", oauth_tenant_id="test")
    ...
    def test_issue_opened_to_model(self):
        with open(get_test_file_path("issues[opened].json"), "r") as f:
            ...
            self.assertEqual(
                model.author_association, data["issue"]["author_association"]
            )
